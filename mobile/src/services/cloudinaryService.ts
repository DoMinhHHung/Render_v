...
import { CLOUDINARY_CONFIG, getAPIURL, API_URL, POSSIBLE_IPS } from "../config/constants";
...
export const uploadImageToCloudinary = async (
  ...
  try {
    ...
    if (!apiUrl || typeof apiUrl !== 'string' || !apiUrl.startsWith('http')) {
      ...
      apiUrl = "http://192.168.1.3:3005";
      console.log("Using fallback API URL:", apiUrl);
    }
    
    console.log("API URL:", `${apiUrl}/api/chat/upload-cloudinary`);
...
export const uploadFile = async (
  ...
  try {
    ...
    try {
      ...
      console.log("Uploading with FormData to:", `${apiUrl}/api/chat/upload-cloudinary`);
